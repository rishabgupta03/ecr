...
CONTROL_NAME = "ECR Repository Has Image Tag Immutability Enabled"
            ...
            for page in paginator.paginate():
                for repo in page.get("repositories", []):
                    ...
                    if tag_mutability == "IMMUTABLE":
                        status = "COMPLIANT"
                        compliant += 1
                        evidence = "Image tag immutability is enabled"
                    else:
                        status = "NON_COMPLIANT"
                        non_compliant += 1
                        evidence = "Image tag immutability is disabled (tags are mutable)"
...
def write_csv(results, account_id):
    filename = f"ecr_repositories_tag_immutability_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check whether ECR repositories have image tag immutability enabled."
    )
