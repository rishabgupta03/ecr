...
# NOTE: This check is marked [DEPRECATED] by the source control list.
# It reflects the legacy per-repository "scan on push" setting, which
# has been superseded by registry-wide scanning configuration
# (see: ecr_registry_scan_images_on_push_enabled). Kept here only for
# accounts/tooling that still reference the deprecated per-repo check.
CONTROL_NAME = "[DEPRECATED] ECR Repository Has Image Scanning On Push Enabled"
            ...
            for page in paginator.paginate():
                for repo in page.get("repositories", []):
                    ...
                    if scan_on_push:
                        status = "COMPLIANT"
                        compliant += 1
                        evidence = "Repository-level scan on push is enabled"
                    else:
                        status = "NON_COMPLIANT"
                        non_compliant += 1
                        evidence = "Repository-level scan on push is disabled"
...
def write_csv(results, account_id):
    filename = f"ecr_repositories_scan_images_on_push_enabled_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="[DEPRECATED] Check whether individual ECR repositories have scan on push enabled."
    )
